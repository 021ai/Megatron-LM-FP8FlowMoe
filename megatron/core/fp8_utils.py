...
if HAVE_TE:
    ...
    def make_fp8_tensor(config, x: torch.Tensor, x_scale: torch.Tensor):
        q = _get_fp8_quantizer(config.fp8_recipe)
        if q is None:
            return None

        if config.fp8_recipe == Fp8Recipe.blockwise:
            ...
            return Float8BlockwiseQTensor(
                ...
                is_scaling_aware_transpose=config.moe_scaling_aware_transpose,
            )
        return None
